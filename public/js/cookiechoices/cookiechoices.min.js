...
var cookieChoices=(function(){var cookieName='displayCookieConsent';var cookieConsentId='cookieChoiceInfo';var dismissLinkId='cookieChoiceDismiss';var dismissIconId='cookieChoiceDismissIcon';function showCookieBar(data){if(typeof data!='undefined'&&typeof data.linkHref!='undefined'){data.position=data.position||"bottom";data.language=data.language||"en";data.styles=data.styles||'position:fixed; width:100%; background-color:rgba(238, 238, 238, 0.9); margin:0; left:0; '+data.position+':0; padding:4px; z-index:1000; text-align:center;';switch(data.language){case "de":data.cookieText=data.cookieText||"Cookies erleichtern die Bereitstellung unserer Dienste. Mit der Nutzung unserer Dienste erklären Sie sich damit einverstanden, dass wir Cookies verwenden. ";data.dismissText=data.dismissText||"OK";data.linkText=data.linkText||"Weitere Informationen";break;case "it":data.cookieText=data.cookieText||"I cookie ci aiutano ad erogare servizi di qualità. Utilizzando i nostri servizi, l'utente accetta le nostre modalità d'uso dei cookie.";data.dismissText=data.dismissText||"OK";data.linkText=data.linkText||"Ulteriori informazioni";break;case "fr":data.cookieText=data.cookieText||"Les cookies nous permettent de vous proposer nos services plus facilement. En utilisant nos services, vous nous donnez expressément votre accord pour exploiter ces cookies.";data.dismissText=data.dismissText||"OK";data.linkText=data.linkText||"En savoir plus";break;case "nl":data.cookieText=data.cookieText||"Cookies helpen ons bij het leveren van onze diensten. Door gebruik te maken van onze diensten, gaat u akkoord met ons gebruik van cookies.";data.dismissText=data.dismissText||"OK";data.linkText=data.linkText||"Meer informatie";break;case "fi":data.cookieText=data.cookieText||"Evästeet auttavat meitä palvelujemme toimituksessa. Käyttämällä palvelujamme hyväksyt evästeiden käytön.";data.dismissText=data.dismissText||"Selvä";data.linkText=data.linkText||"Lisätietoja";break;default:data.cookieText=data.cookieText||"Cookies help us deliver our services. By using our services, you agree to our use of cookies.";data.dismissText=data.dismissText||"Got it";data.linkText=data.linkText||"Learn more"}
_showCookieConsent(data.cookieText,data.dismissText,data.linkText,data.linkHref,data.styles,!1)}}
function _createHeaderElement(cookieText,dismissText,linkText,linkHref,styles){var butterBarStyles=styles;var cookieConsentElement=document.createElement('div');var wrapper=document.createElement('div');wrapper.style.cssText="padding-right: 50px;";cookieConsentElement.id=cookieConsentId;cookieConsentElement.style.cssText=butterBarStyles;wrapper.appendChild(_createConsentText(cookieText));if(!!linkText&&!!linkHref){wrapper.appendChild(_createInformationLink(linkText,linkHref))}
wrapper.appendChild(_createDismissLink(dismissText));cookieConsentElement.appendChild(wrapper);cookieConsentElement.appendChild(_createDismissIcon());return cookieConsentElement}
...
function _setElementText(element,text){var supportsTextContent='textContent' in document.body;if(supportsTextContent){element.textContent=text}else{element.innerText=text}}
...
var exports={};exports.showCookieBar=showCookieBar;exports.showCookieConsentBar=showCookieConsentBar;exports.showCookieConsentDialog=showCookieConsentDialog;return exports})();window.cookieChoices=cookieChoices;return cookieChoices})(this)